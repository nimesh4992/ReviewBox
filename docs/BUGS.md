...
|----|----------|---------|---------|--------|
| BUG-044 | 🔴 CRITICAL | **The 14-day trial shipped with AI drafting switched off.** `PLAN_LIMITS` in `src/lib/plans.ts` had no `trial` key, and both consumers fail closed to `free` for unrecognised plan names — `resolvePlan()` in `plan-enforcement.ts` and `isPlanName()` in `rate-limit.ts`. Onboarding stamps every new user `plan: "trial"`, so every trial user resolved to **free**: `aiDraftsPerDay: 0`, `appsMax: 1`, `reviewsPerMonth: 1_000`. `rate-limit.ts:28` short-circuits on a 0 limit and denies before even reaching Redis, so "Generate draft" failed for 100% of trials. Independent of, and downstream from, BUG-028 — fixing the middleware entitlement gate opened the route and this denied the request one step later, so the earlier fix alone did not restore the feature | `src/lib/plans.ts`, `src/lib/rate-limit.ts`, `src/lib/plan-enforcement.ts` | ✅ Fixed — added a `trial` tier mirroring `pro` (the allowance advertised on /faq and /pricing), plus 3 regression tests asserting trial ≥ free and trial === pro. Unknown plan names still fail closed to `free`. Trial expiry is unaffected, still enforced at `middleware.ts:182` |
| BUG-045 | 🟡 MEDIUM | Trial workspaces displayed as **"free"** in `/admin/customers`. The badge map had its own local `PlanName` union missing `trial`, falling back to `free` — so every new signup looked like a free-tier account in the admin view used to judge MRR and conversion | `src/app/admin/customers/page.tsx` | ✅ Fixed — added a `trial` badge (amber) |
| BUG-046 | 🟡 MEDIUM | `checkReviewLimit()` in `plan-enforcement.ts` is defined and exported but **never called anywhere**, so the advertised monthly review quota (1K/5K/50K by plan) is not enforced at all. Found while auditing the trial gates | `src/lib/plan-enforcement.ts` | Open — decide whether to wire it into the sync path or drop the quota claim from /pricing |
...
| BUG-043 | 🔴 CRITICAL | **Vercel preview deployments return `500 MIDDLEWARE_INVOCATION_FAILED` on every route**, making previews unusable for reviewing PRs. `clerkMiddleware` wraps every path in `config.matcher`, so if Clerk cannot initialise — missing `CLERK_SECRET_KEY` / `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` in the Vercel **Preview** environment scope, or a production Clerk instance key being rejected on a `*.vercel.app` host — the middleware throws before any route handler runs and Vercel serves a 500. Same root design as BUG-040: middleware has no fallback when the auth provider fails. Not introduced by `claude/product-market-readiness-zcfowh`: this branch's only middleware change is inside the handler body, *after* `await auth.protect()`, and locally with middleware enabled `/` and `/pricing` return 200 while `/dashboard` returns 404 — no crash | `src/middleware.ts`, Vercel env config | Open — **HUMAN-REQUIRED diagnostic**: open the preview at `/` (a public route). If `/` also 500s → Clerk env vars are missing from the Preview scope in Vercel → add them. If only `/dashboard` 500s → the Clerk instance rejects the preview host → add the preview domain to the Clerk instance, or use a development instance key for Preview. Durable fix is BUG-040 (let public routes short-circuit before Clerk) |
...
| BUG-022 | 🟠 HIGH | Sync latency is 24h — weak for a "respond fast to bad reviews" product at $99/mo | `vercel.json` | **Fixed by P1-1** (`fix/p0-commercial-readiness`): cron is `0 */3 * * *`, 4h worst-case freshness. The "Vercel Hobby cron cap" cited as the cause was a **false premise** — the project is on Pro (confirmed 2026-08-21), so no `pg_cron` work was needed |
