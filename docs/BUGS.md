...
| BUG-042 | 🟡 MEDIUM | The `Security audit` merge gate conflated "a critical advisory exists" with "npm's registry answered 4xx" — both exit 1. Hit on c844e9d: the legacy `/security/audits/quick` endpoint (which npm's own notice says is being retired) returned `400 Invalid package tree` for a lockfile that had audited clean 44 minutes earlier, blocking a frontend-only PR | `.github/workflows/ci.yml` | ✅ Fixed — retries transient registry errors 3×, still fails on real advisories, and on persistent registry failure passes with a loud `::warning::` marking the run UNVERIFIED rather than silently green |
