...
```bash
npm run eval:score -- --predictions eval/run-groq.csv --name "Groq / LLM"
```
...
The report prints `overall` last and labels it *not a headline*, on purpose.
Reporting "overall 86%" over a 42% Hinglish column is exactly the failure D025
forbids.
