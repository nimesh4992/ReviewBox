...
5. **Middleware matcher gaps break real features on the prod app host.** `/api/import/*`, `/api/competitors/*`, `/api/auth/slack/*`, `/api/cron/*` are in **neither** matcher (`middleware.ts:8-78`); on `app.tryreviewbox.com` unknown paths redirect to `/dashboard` **before** auth runs (`middleware.ts:103-106`) — so AppFollow import, Slack OAuth, and the new Competitors add-flow appear broken in production, and the `trial-nudge` cron is likely blocked by Clerk before its `CRON_SECRET` check (its sibling digests are in the public list; it isn't). *Functional bug as much as an access one.*
