      ...
      <div className="grid grid-cols-3 gap-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="rounded-[14px] border border-[var(--rb-border-1)] bg-surface p-6 space-y-4">
            <div className="h-5 w-24 rounded bg-[var(--rb-bg-sunken)]" />
            <div className="h-9 w-32 rounded bg-[var(--rb-bg-sunken)]" />
            <div className="space-y-2 pt-2">
              {Array.from({ length: 5 }).map((_, j) => (
                <div key={j} className="h-3 w-full rounded bg-[var(--rb-bg-sunken)]" />
              ))}
            </div>
