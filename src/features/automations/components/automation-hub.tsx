...
import { CheckCircle2, Loader2, MoreHorizontal, Plus, Workflow } from "lucide-react";
...
export function AutomationHub() {
  ...
  useEffect(() => {
    ...
    fetch("/api/automations/rules")
      .then(async (res) => {
        if (!res.ok) return; // stay on mock data
  ...
  async function handleInstallPreset(preset: AutomationPreset) {
    ...
    if (res.ok) {
      ...
      // Optimistically add with temp id so rules tab feels responsive
  ...
  async function handleSaveRule(rule: Partial<AutomationRule>) {
    try {
      const res = await fetch("/api/automations/rules", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        // Optimistically add with temp id so UI feels responsive
        const tempRule: AutomationRule = {
          id: `temp-${Date.now()}`,
      ...
      // Same optimistic fallback on network error
      const tempRule: AutomationRule = {
        id: `temp-${Date.now()}`,
        name: rule.name ?? "Untitled rule",
        description: rule.description ?? "",
