...
export default function AiRepliesPage() {
  return (
    <MarketingShell>
      ...
      <Breadcrumb
        trail={[{ label: "Help centre", href: "/help" }, { label: "How AI Replies Work" }]}
      />
      ...
      <main className="mx-auto max-w-screen-xl px-6 pb-32">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_260px] max-w-5xl">
          <div className="space-y-8">
            ...
            {/* Pipeline */}
            <div>
              <h2 className="font-bold text-[var(--rb-fg-1)] text-lg mb-4">The 3-tier pipeline</h2>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-3 mb-2">
                          <h3 className="font-semibold text-[var(--rb-fg-1)]">{tier.label}</h3>
                          <span className="rounded-full bg-[var(--rb-mk-sunken)] px-2.5 py-0.5 text-xs font-medium text-[var(--rb-fg-2)]">
                            {tier.cost}
                          </span>
                        </div>
                        <p className="text-sm text-[var(--rb-fg-2)] leading-relaxed">{tier.description}</p>
                        <p className="mt-2 text-xs font-semibold text-[var(--rb-fg-3)]">{tier.detail}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div id="tones" className="scroll-mt-24">
              <h2 className="font-bold text-[var(--rb-fg-1)] text-lg mb-4">Tone options</h2>
              <div className="rounded-2xl border border-[var(--rb-mk-line)] bg-white divide-y divide-gray-100">
                {TONES.map((tone) => (
                  <div key={tone.name} className="flex items-start gap-4 px-6 py-4">
                    <span className="text-sm font-semibold text-[var(--rb-fg-1)] w-28 shrink-0">{tone.name}</span>
                    <p className="text-sm text-[var(--rb-fg-3)]">{tone.desc}</p>
                  </div>
                ))}
              </div>
              <p className="mt-3 text-xs text-[var(--rb-fg-3)]">
            ...
            <div id="auto-publish" className="scroll-mt-24">
              <h2 className="font-bold text-[var(--rb-fg-1)] text-lg mb-4">Auto-publish rules</h2>
              <div className="rounded-2xl border border-[var(--rb-mk-line)] bg-white p-6">
                ...
                <ul className="mt-4 space-y-2 text-sm text-[var(--rb-fg-2)] list-disc list-inside">
            ...
            <div id="learning" className="scroll-mt-24">
              <h2 className="font-bold text-[var(--rb-fg-1)] text-lg mb-4">Does ReviewBox learn from edits?</h2>
              <div className="rounded-2xl border border-[var(--rb-mk-line)] bg-white p-6">
                <p className="text-sm text-[var(--rb-fg-2)] leading-relaxed">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-[var(--rb-mk-line)] bg-white p-5 text-center">
              ...
              <p className="mt-1 text-xs text-[var(--rb-fg-3)]">Real answers from real people.</p>
