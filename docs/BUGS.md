...
|----|----------|---------|---------|--------|
| BUG-037 | 🟠 HIGH | 11 of 20 e2e specs assert a protected route redirects an anonymous visitor to `/sign-in`, but with an unusable `CLERK_SECRET_KEY` `auth.protect()` returns **404**. Confirmed against a clean `master` worktree — it's the placeholder credentials, not a regression. E2E job is `continue-on-error` until fixed | `.github/workflows/ci.yml`, `tests/e2e/auth-flow.spec.ts` | Open — **HUMAN-REQUIRED**: add Clerk test-instance keys as GitHub repo secrets, then drop `continue-on-error` |
