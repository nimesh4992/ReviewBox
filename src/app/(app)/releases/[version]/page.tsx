...
const STATUS_CONFIG: Record<DerivedStatus, { badge: string; bar: string; label: string }> = {
  degraded:   { badge: "bg-red-50 text-red-700 border border-red-200",               bar: "bg-red-400",    label: "Degraded" },
      ...
      <div className="p-4 md:p-6 space-y-6 max-w-4xl">
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm p-5">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="font-mono text-xl font-semibold text-gray-900">{version}</span>
          ...
          {stats.firstSeen && (
            <p className="mt-2 text-xs text-gray-400">First review {stats.firstSeen}</p>
          )}
        </div>
        ...
        {hasData && stats.topTags.length > 0 && (
          <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">Issue tags</h2>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
          <div className="flex items-center border-b border-gray-100 px-5 py-3">
            <h2 className="text-sm font-semibold text-gray-900">Reviews for v{version}</h2>
            {hasData && (
          ...
          {!hasData ? (
            <div className="px-5 py-8 text-center">
              <MessageSquare className="mx-auto size-8 text-gray-200 mb-2" strokeWidth={1.5} />
            ...
            <ul className="divide-y divide-gray-100">
              {stats.reviews.map((r) => (
                <li key={r.id} className="px-5 py-3 hover:bg-gray-50/40">
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {r.sentiment && (
                          <span className={cn(
                            ...
                            SENTIMENT_BADGE[r.sentiment] ?? "bg-gray-50 text-gray-500",
                          )}>
                            {r.sentiment}
                          </span>
                        )}
                        {r.country && (
                          <span className="text-[10px] text-gray-300">{r.country}</span>
                        )}
                      </div>
                      <p className="text-xs text-gray-500 leading-5 line-clamp-3">{r.body}</p>
                      {(r.issue_tags ?? []).length > 0 && (
                        <div className="mt-1.5 flex flex-wrap gap-1">
                          {(r.issue_tags ?? []).map((t) => (
                            <span
                              key={t}
                              className="rounded-full border border-gray-200 bg-gray-50 px-1.5 py-0.5 text-[10px] text-gray-500"
                            >
                              {TAG_LABELS[t] ?? t}
                            </span>
                          ))}
